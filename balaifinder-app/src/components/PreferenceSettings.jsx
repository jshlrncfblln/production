...
function PreferenceSettings({ onClose, onSubmit }) {
  ...
  const loadData = async () => {
    const [locationResponse, typeResponse, priceResponse, nearMallResponse, nearSchoolResponse, nearChurchResponse] = await Promise.all([
    ...
    <>
      <div className="fixed inset-0 z-50 flex items-center justify-center bg-gray-900 bg-opacity-50">
        <div className="bg-white p-6 rounded-md shadow-md relative">
          ...
          <form onSubmit={handleSubmit} className="" method="POST" action="https://balaifinder-backend-deploy.onrender.com/api/post/submitpreferences">
            ...
            <div className="grid grid-cols-2 gap-y-8 gap-x-24">
              <div className="flex flex-col mb-4">
                ...
                <select name="price" value={formData.price} onChange={handleChange} className="border-sky-500 border rounded-lg py-2">
                  <option disabled hidden value="">Select Property Price</option>
            ...
            <select name="location" value={formData.location} onChange={handleChange} className="border-sky-500 border-[3px] rounded-full">
        ...
        <div className="flex flex-col mb-4">
            ...
            <select name="house_type" value={formData.house_type} onChange={handleChange} className="border-sky-500 border-[3px] rounded-full">
        ...
        <div className="flex flex-col mb-4">
            ...
            <select name="near_school" value={formData.near_school} onChange={handleChange} className="border-sky-500 border-[3px] rounded-full">
                <option disabled hidden value="">Select</option>
                {nearSchoolData.map((item, index) => (
                <option key={index} value={item.isnearschool}>{item.isnearschool}</option>
                ))}
            </select>
        </div>
        <div className="flex flex-col mb-4">
